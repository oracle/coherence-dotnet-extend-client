...
{
    [TestFixture]
    public class AggregateAllRequestTest : BaseNamedCacheTest
    {
        public AggregateAllRequest CreateRequest()
        {
            ...
            try
            {
                return (AggregateAllRequest)namedCache.MessageFactory.CreateMessage(AggregateAllRequest.TYPE_ID);
            }
            finally
            {
                conn.Close();
