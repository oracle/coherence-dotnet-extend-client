...
{
    [TestFixture]
    public class InvokeFilterRequestTest : BaseNamedCacheTest
    {
        public InvokeFilterRequest CreateRequest()
        {
            ...
            try
            {
                return (InvokeFilterRequest)namedCache.MessageFactory.CreateMessage(InvokeFilterRequest.TYPE_ID);
            }
            finally
            {
                conn.Close();
