...
{
    [TestFixture]
    public class InvokeRequestTest : BaseNamedCacheTest
    {
        public InvokeRequest CreateRequest()
        {
            ...
            try
            {
                return (InvokeRequest) namedCache.MessageFactory.CreateMessage(InvokeRequest.TYPE_ID);
            }
            finally
            {
                conn.Close();
