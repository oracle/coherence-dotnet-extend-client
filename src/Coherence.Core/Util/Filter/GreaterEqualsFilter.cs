...
namespace Tangosol.Util.Filter
{
    ...
    public class GreaterEqualsFilter : ComparisonFilter, IIndexAwareFilter
    {
        #region Constructors
        ...
        public IFilter ApplyIndex(IDictionary indexes, ICollection keys)
        {
            ...
            if (index.IsOrdered)
            {
                ...
                {
                    var setGE = new HashSet();
                    foreach (ICollection col in sortedGE.Values)
                    {
                        CollectionUtils.AddAll(setGE, col);
                    }
                    CollectionUtils.RetainAll(keys, setGE);
                }
                else
                {
            ...
            else
            {
                ...
                if (index.IsPartial)
                {
                    ...
                    foreach (DictionaryEntry entry in contents)
                    {
                        ...
                        {
                            CollectionUtils.AddAll(setGE, (ICollection) entry.Value);
                        }
                    }
                    CollectionUtils.RetainAll(keys, setGE);
                }
