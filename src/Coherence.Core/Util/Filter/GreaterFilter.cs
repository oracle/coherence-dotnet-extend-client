...
namespace Tangosol.Util.Filter
{
    ...
    public class GreaterFilter : ComparisonFilter, IIndexAwareFilter
    {
        #region Constructors
        ...
        /// </param>
        public GreaterFilter(IValueExtractor extractor, IComparable value)
        ...
        /// The object to compare the result with.
        /// </param>
        public GreaterFilter(string member, IComparable value)
            : base(member, value)
        {}
        ...
        /// </param>
        public GreaterFilter(string member, int value)
        ...
        /// </param>
        public GreaterFilter(string member, long value)
            : this(member, (IComparable) value)
        {}
        ...
        /// </param>
        public GreaterFilter(string member, float value)
            : this(member, (IComparable) value)
        {}
        ...
        /// </param>
        public GreaterFilter(string member, double value)
            : this(member, (IComparable) value)
        {}
        ...
        public IFilter ApplyIndex(IDictionary indexes, ICollection keys)
        {
            ...
            if (index.IsOrdered)
            {
                ...
                var colEquals = (ICollection) contents[value];
                ...
                {
                    var setGT = new HashSet();
                    foreach (ICollection col in sortedGE.Values)
                    {
                        if (col != colEquals)
                        {
                            CollectionUtils.AddAll(setGT, col);
                        }
                    }
                    CollectionUtils.RetainAll(keys, setGT);
                }
                else
                {
                    ...
                    if (colEquals != null)
                    {
                        CollectionUtils.RemoveAll(keys, colEquals);
                    }
                }
                // the NULL set doesn't get in
            }
            else
            {
                ...
                if (index.IsPartial)
                {
                    ...
                    foreach (DictionaryEntry entry in contents)
                    {
                        ...
                        {
                            CollectionUtils.AddAll(setGT, (ICollection) entry.Value);
                        }
                    }
                    CollectionUtils.RetainAll(keys, setGT);
                }
